...
export const loader = async ({params}) => {
    ...
    return json(({
        state: await getStateById({stateId, url: "/high/results"}),
        results: await getTopStudentsByStateId({stateId, resultId, url: "/high"})
    }))
}
...
export async function action({ request, params }) {
    ...
    switch (action) {
        case "search": {
            ...
            const isTitle = true
            return json(await getResultStudent({page, resultId, target, value, isTitle, searchBy, searchById: stateId, url: "/high"}));
        }
        default:
            return json({})
    }
