...
import {
    CreateResultSettings, deleteSession, getSessions,
...
export const meta = ({ matches }) => {
    return generatePageTitle({matches, current: "الدورات"});
};

export const handle = {
    breadcrumb: () => <Link className={"text-indigo-700 hover:text-indigo-600"} to="/dash/results/sessions">الدورات</Link>,
};

export default function DashResultsSessions_index() {
    const {status, data: sessions} = useLoaderData();
    ...
    return (
        <Section>
            {
                showAdd &&
                <CardAdd isSubmitting={isSubmitting} onSubmit={submit} onClick={() => setShowAdd(false)} title={isEdit ? "تعديل دورة" : "إضافة دورة"}>
                        <Input isError={error?.name} onChange={(e) => setName(e.target.value)} defaultValue={name} type={"text"} name={"name"} label={"الاسم"} placeholder={"اكتب اسم الدورة"} />
                        {
            ...
            <div className={"flex items-center justify-between gap-4"}>
                <PageTitle title={"الدورات"}/>
                <Button onClick={() => setShowAdd(true)} title={"إضافة دورة"}/>
            </div>
            {
                status === "success" &&
                                ...
                                <Td value={
                                    <div className={"flex justify-center items-center gap-3"}>
                                        <EditIcon onClick={ async () => {
                                            ...
                                            setIsEdit(true)
                                        }} isLoading={isSubmitting} currentId={currentId} itemId={session.id} />
...
export async function action({ request }) {
    ...
    if(user && user.role === "ADMIN"){
        switch (action) {
            case "create": {
                return json( await CreateResultSettings({name, slug, model: "session"}));
            }
            case "edit": {
                return json( await updateResultSettings({id, name, slug, model: "session"}));
            }
            case "delete": {
                return json( await deleteSession(id));
            }
        }
    } else {
        return redirect("/")
