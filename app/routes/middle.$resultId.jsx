...
export const loader = async ({params}) => {
    ...
    return json(await getResultById({resultId, url: "/middle/results"}))
}
...
export default function MiddleResultsResultId() {
    const result = useLoaderData();
    return (
        <section className={"flex px-5 flex-col gap-3 mb-3"}>
            <ResultsTop url={"middle"} type={result.type} session={result.session} year={result.year} />
            <hr />
            <Outlet/>
        </section>
    )
