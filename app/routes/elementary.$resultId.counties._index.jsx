...
export const loader = async ({request, params}) => {
    ...
    return json(await getCountiesByResultId({resultId, sort, url: "/elementary/results"}))
}
...
export const handle =  {
    breadcrumb: () => <Link className={"text-indigo-700 hover:text-indigo-600"} to="/elementary/results/counties">المقاطعات</Link>,
};
export async function action({ request, params }) {
    ...
    switch (action) {
        case "pagination": {
            return json(await getCountiesByResultId({resultId, page, sort, url: "/elementary/results"}));
        }
        case "search": {
            return json(await searchCounties({resultId, value, sort, url: "/elementary/results"}));
        }
        default:
            return json({});
    }
...
export default function ElementaryResultsResultIdCounties_index() {
