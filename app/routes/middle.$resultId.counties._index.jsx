...
export const loader = async ({request, params}) => {
    ...
    return json(await getCountiesByResultId({resultId, sort, url: "/middle/results"}))
}
...
export const handle =  {
    breadcrumb: () => <Link className={"text-indigo-700 hover:text-indigo-600"} to="/middle/results/counties">المقاطعات</Link>,
};
export async function action({ request, params }) {
    ...
    switch (action) {
        case "pagination": {
            return json(await getCountiesByResultId({resultId, page, sort, url: "/middle/results"}));
        }
        case "search": {
            return json(await searchCounties({resultId, value, sort, url: "/middle/results"}));
        }
        default:
            return json({});
    }
...
export default function MiddleResultsResultIdCounties_index() {
