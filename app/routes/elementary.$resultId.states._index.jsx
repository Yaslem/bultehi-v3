...
export const loader = async ({params, request}) => {
    ...
    return json(await getStatesByResultId({sort, resultId, url: "/elementary/results"}))
}
...
export async function action({ request, params }) {
    ...
    switch (action) {
        case "search": {
            return json(await searchStates({resultId, value, sort, url: "/elementary/results"}));
        }
        default:
            return json({});
    }
...
export const handle =  {
    breadcrumb: () => <Link className={"text-indigo-700 hover:text-indigo-600"} to="/elementary/results/schools">الولايات</Link>,
};

export default function MiddleResultsResultIdStates_index() {
