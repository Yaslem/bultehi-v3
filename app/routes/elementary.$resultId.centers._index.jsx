...
import {
    getCentersByResultId, searchCenters
...
export const loader = async ({request, params}) => {
    ...
    return json(await getCentersByResultId({resultId, sort, url: "/elementary/results"}))
}
...
export const handle =  {
    breadcrumb: () => <Link className={"text-indigo-700 hover:text-indigo-600"} to="/elementary/results/centers">المراكز</Link>,
};
export async function action({ request, params }) {
    ...
    switch (action) {
        case "pagination": {
            return json(await getCentersByResultId({resultId, page, sort, url: "/elementary/results"}));
        }
        case "search": {
            return json(await searchCenters({resultId, value, sort, url: "/elementary/results"}));
        }
        default:
            return json({});
    }
...
export default function ElementaryResultsResultIdCenters_index() {
