...
export const loader = async ({request, params}) => {
    ...
    return json(await getSchoolsByResultId({resultId, sort, url: "/elementary/results"}))
}
...
export const handle =  {
    breadcrumb: () => <Link className={"text-indigo-700 hover:text-indigo-600"} to="/elementary/results/schools">المدارس</Link>,
};
export async function action({ request, params }) {
    ...
    switch (action) {
        case "pagination": {
            return json(await getSchoolsByResultId({resultId, page, sort, url: "/elementary/results"}));
        }
        case "search": {
            return json(await searchSchools({resultId, value, sort, url: "/elementary/results"}));
        }
        default:
            return json({});
    }
...
export default function ElementaryResultsResultIdSchools_index() {
