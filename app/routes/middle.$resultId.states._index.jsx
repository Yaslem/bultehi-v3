...
export const loader = async ({params, request}) => {
    ...
    return json(await getStatesByResultId({sort, resultId, url: "/middle/results"}))
}
...
export async function action({ request, params }) {
    ...
    switch (action) {
        case "search": {
            return json(await searchStates({resultId, value, sort, url: "/middle/results"}));
        }
        default:
            return json({});
    }
...
export const handle =  {
    breadcrumb: () => <Link className={"text-indigo-700 hover:text-indigo-600"} to="/middle/results/schools">الولايات</Link>,
};
